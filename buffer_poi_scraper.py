"""
Buffer POI Scraper - Google Maps (No API Key)
Grid-based search within the All_Buffered shapefile boundary.
...
# CONFIG
# ─────────────────────────────────────────
SHAPEFILE_PATH = "Bangkok shapefile/All_Buffered.shp"
OUTPUT_CSV     = "buffer_poi.csv"
...
GRID_SPACING_M = 500    # 500 m spacing — finer grid for buffer zones
SEARCH_ZOOM    = 15     # Google Maps zoom level — 15 ≈ ~1 km radius visible on screen
SCROLL_TIMES   = 15     # Number of scrolls per result list
...
# GRID GENERATION
# ─────────────────────────────────────────
def generate_grid_points(shapefile_path: str, spacing_m: float):
    """
    Build a regular grid of WGS84 (lat, lon) points that fall inside the
    dissolved buffer boundary.  Grid is constructed in the projected CRS
    (EPSG:32647) so spacing is uniform in metres, then reprojected to WGS84.
    """
    gdf = gpd.read_file(shapefile_path)
    # Shapefile already carries EPSG:32647 — no override needed

    # Dissolve all 192 buffer polygons into a single union geometry
    ...
    pts_proj = [Point(x, y) for x in xs for y in ys if boundary.contains(Point(x, y))]

    pts_gdf = gpd.GeoDataFrame(geometry=pts_proj, crs=crs)
...
# ─────────────────────────────────────────
async def main():
    grid_points = generate_grid_points(SHAPEFILE_PATH, GRID_SPACING_M)
